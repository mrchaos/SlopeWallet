...
typedef TopSheetDragStartHandler = void Function(DragStartDetails details);
typedef TopSheetDragEndHandler = void Function(
...
///    function.
///
/// The [TopSheet] widget itself is rarely used directly. Instead, prefer to
/// create a persistent bottom sheet with [ScaffoldState.showBottomSheet] or
/// [Scaffold.bottomSheet], and a modal bottom sheet with [showModalTopSheet].
///
/// See also:
///
///  * [showTopSheet] and [ScaffoldState.showBottomSheet], for showing
///    non-modal "persistent" bottom sheets.
///  * [showModalTopSheet], which can be used to display a modal bottom
///    sheet.
...
class TopSheet extends StatefulWidget {
  ...
  final TopSheetDragStartHandler? onDragStart;
  ...
  /// sheet is closing.
  final TopSheetDragEndHandler? onDragEnd;
...
class _ModalTopSheetLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    return Offset(0.0, childSize.height * (progress! - 1));
  }
...
class _ModalTopSheetRoute<T> extends PopupRoute<T> {
  ...
  AnimationController createAnimationController() {
    ...
    _animationController =
        TopSheet.createAnimationController(navigator!.overlay!);
...
/// can be created and displayed with the [showTopSheet] function or the
...
/// display the [TopSheet] when set to `true`. This is useful in the case
/// that a modal [TopSheet] needs to be displayed above all other content
...
/// See also:
///
///  * [TopSheet], which becomes the parent of the widget returned by the
...
Future<T?> showModalTopSheet<T>({
  ...
  assert(debugCheckHasMediaQuery(context));
  assert(debugCheckHasMaterialLocalizations(context));
  var appColors = context.read<AppTheme>().currentColors;
  barrierColor = appColors.barrierColor;

  return Navigator.of(context, rootNavigator: useRootNavigator)
      .push(_ModalTopSheetRoute<T>(
    builder: builder,
...
Future<T?> showTopActionSheet<T>({
  required BuildContext context,
  WidgetBuilder? builder,
}) {
  return showModalTopSheet<T>(
      context: context,
...
Widget get kThumb => Builder(builder: (context) {
      var appColors = context.read<AppTheme>().currentColors;
      return Container(
        width: 32,
        height: 4,
...
Future<T?> showBottomPageSheet<T>({
  required BuildContext context,
  WidgetBuilder? builder,
  ...
  return showModalBottomSheet<T>(
      ...
      builder: (c) {
        ...
        if (constraints != null) {
          page = ConstrainedBox(constraints: constraints, child: page);
        }
